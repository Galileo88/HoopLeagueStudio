...
Displayed ranks compare each team's valid saved lineup, with temporary exhibition preparation used only when a lineup needs it for rating calculation. Persisted depth-chart rebuilds intentionally rank players by the **current player star rating (attribute index 0) only**; stored potential (`pot`) does not affect starter or backup ordering. New randomized teams and expansion teams are rebuilt immediately with this current-overall ordering, including valid but inherited template slots. Importing or loading a league now rebuilds every supported depth chart with the same current-overall ordering, including lineups that were already valid. Updated roster order, team positions, position ranks, and matching startingLineup player references are saved and exported; attributes and player identities are unchanged. Reloading an already rebuilt league is idempotent. Franchise injuries, redshirts, career-performance modifiers and scouting visibility remain outside this custom-league scope.
...
The native lineup fixtures remain a regression reference for the temporary exhibition-preparation helper. Persisted editor depth charts intentionally differ from that native optimizer by excluding potential and ordering lineup rebuilds from current overall only.
