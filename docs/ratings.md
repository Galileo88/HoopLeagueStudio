...
Displayed ranks compare the selected team's prepared lineup against the other teams' saved lineups. This is a deterministic exhibition preview, not a simulation of the user's game-session navigation history: Hoop Land repairs the two selected teams in memory, so an opponent selection or previously repaired teams can change its later ranks. Rank tooltips identify this comparison. Franchise injuries, redshirts, career-performance modifiers and scouting visibility are outside this custom-league preview's scope. No calculated lineup, position, rating, or star value is written into league exports.
